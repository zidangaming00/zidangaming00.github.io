/**
 * SEARCH ENGINE CORE - Refactored & Modernized (Unified Tab-Result Architecture)
 */
...
const Utils = {
    ...
    loadCSS: (filename) => {
        ...
        const defStyle = document.querySelector("link[rel='stylesheet']");
        if (defStyle) Utils.insertAfter(defStyle, link);
...
const Widgets = {
    renderInstantCard: (res) => {
        ...
        container.className = "tab-result instant-answer";
        container.innerHTML = `
        ...
        const wrapper = Config.windowWidth > 780 ? document.querySelector(".sidebar-panel") || (() => {
            ...
            side.className = "sidebar-panel";
            document.querySelector(".result-wrapper").appendChild(side);
            return side;
        })() : document.querySelectorAll(".tab-result")[2];
    ...
    renderWidgets: () => {
        ...
        const mainResult = document.querySelector(".main-result .result");
        ...
        if (isTime) {
            ...
            mainResult.insertAdjacentHTML('beforeend', `<div class="tab-result widget-card"><div class="big-title">${timeStr}</div><div class="snippet-info">${dateStr}</div></div>`);
        } else if (isDate) {
            mainResult.insertAdjacentHTML('beforeend', `<div class="tab-result widget-card"><div class="big-title">${d.toLocaleDateString(localLang, { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}</div></div>`);
        } else if (isCalc) {
            mainResult.insertAdjacentHTML('beforeend', `
                <div class="tab-result calculator-card">
            ...
            mainResult.insertAdjacentHTML('beforeend', `
                <div class="tab-result translate-card">
    ...
    checkVideoWidget: async () => {
        ...
        try {
            ...
            for (let i = 0; i < limit; i++) {
                ...
                videonya += `
                    <div class="tab-result video-item">
            ...
            slot.className = "tab-result video-widget-wrapper";
    ...
    initTranslator: () => {
        ...
        exchangeIcon.addEventListener("click", () => {
            let tempVal = fromText.value;
            fromText.value = toText.value;
            toText.value = tempVal;
            let tempLang = selects[0].value;
            selects[0].value = selects[1].value;
            selects[1].value = tempLang;
            translate();
        });
...
const UI = {
    renderBase: () => {
        ...
        document.body.innerHTML = `
            ...
            <style>
                .logo#main-lgx { display: flex; justify-content: center; align-items: center; background: #fff; padding: 8px 12px; margin: 0; width: 100%; }
                .logo#main-lgx img { width: 100px; height: 45px; object-fit: contain; }
                @media (min-width: 780px) { .logo#main-lgx { justify-content: flex-start !important; padding-left: 16px; } }
            </style>
        `;
    ...
    setupEventListeners: () => {
        ...
        document.addEventListener('click', e => {
            if (e.target.matches('.show-wrapper .more')) handlePagination();
        });
    },
    setupTabStyles: () => {
        ...
        if (Config.tbm === "vid") {
            ...
            mainResult.innerHTML += `<div class="UoZrQ result"></div>`;
            Utils.loadCSS("e8495");
        }
    },
    renderFooter: () => {
    ...
    renderEmptyState: () => {
        ...
        document.querySelector(".main-result").innerHTML += `
            <div class="tab-result empty-card">
...
function renderVideos(res) {
    ...
    res.items.forEach(item => {
        container.insertAdjacentHTML('beforeend', `
            <div class="tab-result video-card">
...
function renderNews(res) {
    ...
    res.items.forEach(item => {
        ...
        container.insertAdjacentHTML('beforeend', `
            <div class="tab-result news-card">
                <div class="snwt">
                    <a href="${item.link}">
                        ${thumb}
                        <div class="top">
                            <img src="https://t0.gstatic.com/faviconV2?client=SOCIAL&type=FAVICON&url=${item.link}&size=64" class="favicon">
...
function renderWebResults(res) {
    ...
    if (isFirstPage) {
        if (Config.windowWidth > 700) {
            document.querySelector(".main-result").insertAdjacentHTML('afterbegin', `
                <div class="WsXZp">
                    <div class="result-stats">${isIdLang ? `Sekitar ${res.searchInformation.formattedTotalResults} hasil (${res.searchInformation.formattedSearchTime} detik)` : `Approximately ${res.searchInformation.formattedTotalResults} result (${res.searchInformation.formattedSearchTime} seconds)`}</div>
                </div>
            `);
        }
        if (res.spelling) {
            container.insertAdjacentHTML('beforeend', `
                <div class="tab-result spelling-card">
                    <div class="snippet">${getText("correct")} <a href="/search?q=${encodeURIComponent(res.spelling.correctedQuery)}${searchLangParam}">${res.spelling.correctedQuery}</a><span>?</span></div>
                </div>
            `);
        }
        ...
        container.insertAdjacentHTML('beforeend', `
            <div class="tab-result web-card">
                <div class="CeWka NbkAw">
                    ...
                    <div class="btm-snpt">
                        <div class="snippet rawr"><span>${snippet || getText("noSiteInfo")}</span></div>
                    </div>
                </div>
            </div>
        `);

        // Sisipkan slot kosong tepat setelah hasil web ke-2 (Indeks 1)
        if (i === 1 && isFirstPage) {
    ...
    if (res.queries?.nextPage && isFirstPage) {
        document.querySelector(".main-result").insertAdjacentHTML('beforeend', `
            <div class="show-wrapper"><button class="more">${getText("more")}</button></div>
        `);
    }
    ...
    if (isFirstPage) {
        ...
        API.fetchSuggestions(Config.q).then(sug => {
            if (sug.suggestions && sug.suggestions.length) {
                ...
                container.insertAdjacentHTML('beforeend', `
                    <div class="related-search tab-result">
                        <div class="YjKdl"><div class="title">${getText("related")}</div></div>
    ...
    if (settings.newtab) {
        document.querySelectorAll(".main-result a").forEach(a => a.target = "_blank");
    }
}
